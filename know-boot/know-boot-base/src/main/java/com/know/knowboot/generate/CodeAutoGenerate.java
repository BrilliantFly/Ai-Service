package com.know.knowboot.generate;
...
public class CodeAutoGenerate {
     ...
     * 通过Window窗口进行代码生成
     */
    public void generateCodeByWindow(){
        new CodeWindow().pack();
    }
     ...
     * 单个表代码生成
     */
    public static void generateOneCode(){
        //第二种生成方式
        TableVo mainTable = new TableVo();
        //表名
        mainTable.setTableName("zb_jj_cate");
        //实体名
        mainTable.setEntityName("ZbJjCate");
        // 主键
		mainTable.setSequenceCode("id");
        //包名
        mainTable.setEntityPackage("base");
        //描述
        mainTable.setFtlDescription("基础任务表");
        try {
            new CodeGenerateOne(mainTable).generateCodeFile(null);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
     ...
     * 一对多(父子表)数据模型
     */
    public void generateOneToMainCode(){
        //第一步：设置主表配置
